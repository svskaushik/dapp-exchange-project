...
contract('Token', ([deployer, receiver, exchange]) => {
  ...
  describe('sending tokens', () => {
    ...
    describe('failure', () => {
      it('rejects insufficient balances', async () => {
        ...
        let invalidAmount
        //Attempt transferring amount greater than total supply
        invalidAmount = tokens(100000000) // 100 Million - greater than total supply
        await token.transfer(receiver, invalidAmount, {from: deployer}).should.be.rejectedWith(EVM_REVERT)
        //Attempt transferring tokens when you have none
        await token.transfer(deployer, invalidAmount, {from: receiver}).should.be.rejectedWith(EVM_REVERT)
      })

      it('rejects invalid recepients', async() => {
        await token.transfer(0x0, amount, {deployer}).should.be.rejected
      })
    })
  })
  ...
  describe('delegated token transfers', () => {
    ...
    describe('failure', () => {
      ...
      it('rejects invalid recepients', async() => {
        await token.transferFrom(deployer, 0x0, amount, {deployer}).should.be.rejected
      })
    })
  })
})